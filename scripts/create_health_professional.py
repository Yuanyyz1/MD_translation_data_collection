...
def create_or_get_health_professional(email: str) -> User:
    db = SessionLocal()
    try:
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python scripts/create_health_professional.py health_professional1@example.com [health_professional2@example.com ...]")
        raise SystemExit(1)
    ...
    for raw_email in sys.argv[1:]:
        ...
        if not email:
            continue
        user = create_or_get_health_professional(email)
        print(f"{email} -> {BASE_URL}/health-professional/{user.access_token}/tasks")
