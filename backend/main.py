...
from .crud import (
    clear_all_doctor_tasks,
    clear_submitted_output,
    clear_uploaded_conversations,
    get_admin_metadata_rows,
    get_doctor_dataset_export_rows,
    get_doctor_progress_rows,
...
app = FastAPI(title="Bilingual Doctor Annotation Prototype")
...
def get_workspace_screenshot_path(doctor_id: int, dataset_name: str) -> Path:
    safe_dataset = slugify_filename_part(dataset_name)
    return WORKSPACE_SCREENSHOT_DIR / f"doctor_{doctor_id}__dataset_{safe_dataset}.png"
...
@app.get("/doctor/{token}/tasks")
def doctor_tasks(
    token: str,
    request: Request,
    db: Session = Depends(get_db),
):
    current_user = require_user_by_token(db, token, "doctor")
    conversations = db.scalars(select(Conversation)).all()
    submissions = db.scalars(select(Submission).where(Submission.doctor_id == current_user.id)).all()
    ...
    return templates.TemplateResponse(
        "doctor_datasets.html",
        {"request": request, "current_user": current_user, "dataset_rows": dataset_rows, "access_token": token},
    )


@app.get("/doctor/{token}/tasks/{dataset_name}")
def doctor_tasks_for_dataset(
    token: str,
    dataset_name: str,
    request: Request,
    db: Session = Depends(get_db),
    ...
    def speaker_category(speaker: str) -> str:
        ...
        if any(label in speaker_value for label in ("pharmacist", "doctor", "clinician", "provider", "nurse")):
            return "clinician"
...
@app.post("/doctor/{token}/workspace-screenshot")
...
@app.get("/doctor/{token}/annotate/{conversation_id}")
def doctor_annotate(
    token: str,
    conversation_id: str,
    request: Request,
    ...
    return templates.TemplateResponse(
        "doctor_annotate.html",
        {
            ...
            "access_token": token,
            "doctor_base_path": f"/doctor/{token}",
        },
    )


@app.post("/doctor/{token}/submission/{conversation_id}/save-draft")
def save_draft(
    token: str,
    conversation_id: str,
...
@app.post("/doctor/{token}/submission/{conversation_id}/submit")
...
@app.post("/doctor/{token}/submission/{conversation_id}/discard")
def discard_draft(
    token: str,
    conversation_id: str,
    db: Session = Depends(get_db),
...
@app.get("/doctor/{token}/submission/{conversation_id}/annotations")
...
@app.post("/doctor/{token}/submission/{conversation_id}/annotations")
def add_annotation(
    token: str,
    conversation_id: str,
...
@app.post("/doctor/{token}/submission/{conversation_id}/annotations/{annotation_id}/delete")
...
def admin_upload_page(
    ...
    doctor_progress_rows = get_doctor_progress_rows(db)
    doctor_dataset_export_rows = get_doctor_dataset_export_rows(db)
    return templates.TemplateResponse(
        "admin_upload.html",
        {
            "request": request,
            ...
            "doctor_progress_rows": doctor_progress_rows,
            "doctor_dataset_export_rows": doctor_dataset_export_rows,
            "access_token": token,
        },
    )
...
def admin_download_workspace_screenshot(
    ...
    doctor = db.scalar(select(User).where(User.email == normalized_email, User.role == "doctor"))
    if not doctor:
        raise HTTPException(status_code=404, detail="Doctor not found")

    screenshot_path = get_workspace_screenshot_path(doctor.id, dataset_name)
...
async def admin_upload_submit(
    ...
    rows = list(reader)
    inserted, updated = upsert_conversations_from_rows(db, rows, normalized_dataset_name)
    message = (
...
@app.post("/admin/{token}/clear-doctor-tasks")
def admin_clear_doctor_tasks(
    token: str,
    request: Request,
    db: Session = Depends(get_db),
):
    current_user = require_user_by_token(db, token, "admin")
    counts = clear_all_doctor_tasks(db)
    message = (
        "Doctor task cleanup complete. "
...
@app.get("/admin/{token}/export/doctor/{doctor_email}")
def admin_export_csv_by_doctor(
    token: str,
    doctor_email: str,
    db: Session = Depends(get_db),
):
    _ = require_user_by_token(db, token, "admin")
    rows = get_submitted_export_rows(db, doctor_email=doctor_email)
    ...
    safe_email = "".join(ch if ch.isalnum() or ch in {"@", ".", "_", "-"} else "_" for ch in doctor_email)
    output.seek(0)
...
@app.get("/admin/{token}/export/doctor/{doctor_email}/dataset/{dataset_name}")
def admin_export_csv_by_doctor_dataset(
    token: str,
    doctor_email: str,
    dataset_name: str,
    db: Session = Depends(get_db),
    ...
    _ = require_user_by_token(db, token, "admin")
    rows = [
        row for row in get_submitted_export_rows(db, doctor_email=doctor_email)
...
@app.get("/admin/{token}/export-by-doctor-dataset")
def admin_export_csv_by_doctor_dataset_query(
    token: str,
    doctor_email: str = Query(...),
    dataset_name: str = Query(...),
    db: Session = Depends(get_db),
